// Para Minecraft Bedrock Edition (usando @minecraft/server)
import { world } from "@minecraft/server"; 

// Para Minecraft Java Edition, com Spigot/Paper, você precisaria usar algo como:
...
class Commands {
    ...
    // Para Java Edition, adaptação do comando seria algo como:
    // public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    //     Bukkit.dispatchCommand(sender, "comando_aqui");
    //     return true;
    // }
}
...
world.beforeEvents.chatSend.subscribe(data => {
    try {
        const emoji = { // Lista de emojis personalizáveis
        ...
        // Substituindo os emojis nas mensagens de chat
        data.message = Object.keys(emoji).forEach(key => data.message = data.message.replaceAll(`${key}`, emoji[key]));
    } catch (error) {
        console.warn(`${error}, ${error.stack}`); // Para debugging
        return (data.cancel = false); // Continua processando a mensagem
    }
});
